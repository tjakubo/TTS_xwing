...
function SBM_clickBuilder()
    ...
    local iNote = TM.Instantiate('[b]INSTRUCTIONS[/b]', Vect_Sum(sPos, {-17, 0.5, 22}), {1.5, 1.5, 1.5}, {0, -90, 0})
    iNote.setLuaScript('')
    local eNote = TM.Instantiate('Example squad to paste', Vect_Sum(sPos, {-10, 0.5, 22}), {1.5, 1.5, 1.5}, {0, -90, 0})
    eNote.setLuaScript('')
...
function SBM_clickCollection()
    ...
    for k,info in pairs(items) do
        ...
        SM_sPos = Vect_Sum(SM_sPos, {-2, 0, 0})
        TempModule.AddChildren('Ship Models Collection', newObj)
    end
    --TM.DeleteTempChildren('Ship Models Collection')
    local PC_bag = TM.Instantiate('Pilot Cards Collection', Vect_Sum(sPos, {0, -5, 0}))
    PC_bag.lock()
    ...
    for k,info in pairs(items) do
        ...
        PC_sPos = Vect_Sum(PC_sPos, {0, 0, -3})
